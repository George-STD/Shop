...
const Header = () => {
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'header-scrolled' : 'bg-white shadow-sm'}`}
      role="banner"
    >
      {/* Top Bar */}
      <div
        className={`topbar-gradient text-white text-sm transition-all duration-300 overflow-hidden ${isScrolled ? 'py-0 max-h-0 opacity-0' : 'py-2 max-h-12 opacity-100'}`}
      >
        <div className="container-custom flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-4" aria-label={STRINGS.NAV.QUICK_LINKS}>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            >
              ...
              {cartCount > 0 && (
                <span
                  className="absolute -top-0.5 -right-0.5 bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-sm animate-[pulse-ring_2s_ease-in-out_infinite]"
                  aria-hidden="true"
                >
                  {cartCount}
                </span>
      ...
      <nav
        className={`hidden lg:block border-t border-gray-100 transition-all duration-300 ${isScrolled ? 'max-h-0 overflow-hidden opacity-0' : 'max-h-16 opacity-100'}`}
